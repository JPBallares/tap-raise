...
typedef struct cr_card_info_t {
    ...
    /// First 6 or 8 digits of PAN is the IIN, otherwise known as BIN. NULL terminated string.
    char issuer_identification_number[CR_PAYMENT_PAN_IIN_PREFIX_LENGTH + 1];
    /// Expiry year of card in YY.
    uint8_t card_expiry_year;
    /// Expiry date of card in MM (01-12).
    uint8_t card_expiry_month;
    /// The issuing bank of the card if recognized. This differs from `issuer_id` which reflects the issuing brand (ex. amex, visa).
