...
enum
{
    CR_PAYMENT_UID_MAX_LENGTH = 10,
    CR_PAYMENT_MERCHANT_ID_MIN_LENGTH = 13,
    CR_PAYMENT_MERCHANT_ID_MAX_LENGTH = 32,
...
// Callback for Record Read and write complete
//
//'context' Context pointer.
//'payment' Payment pointer.
//'application_type' It is returned to verify the correct application type is read.
//'merchant_id' it is returned to verify the correct merchant id is read.
//'merchant_id_length' Actual length of merchant_id
//'uid' UID of the card is returned since it can be used for cryptographic operations, like encryption and description.
// Its length is always 10 bytes for mifare cards. 'uid_length' Actual length of the UID 'record_idx' it is returned to
// verify the correct record is read. 'total_records' It indicates if there are more records to be read. 'record' Record
// stored in the card 'result'  Result of the read operation. If that is not successful, record and record_idx should be
// ignored.
typedef void (*cr_payment_record_request_complete_t)(void *context,
                                                     cr_payment_t *payment,
                                                     cr_payment_record_application_type_t application_type,
                                                     uint8_t merchant_id[CR_PAYMENT_MERCHANT_ID_MAX_LENGTH],
                                                     size_t merchant_id_length,
                                                     uint8_t uid[CR_PAYMENT_UID_MAX_LENGTH],
                                                     size_t uid_length,
                                                     uint8_t record_idx,
...
typedef struct cr_payment_event_api_t
{
    ...
    // Callback for Read/Write record completion
    cr_payment_record_request_complete_t on_record_request_complete;

    // Callback for secure Read/Write record completion
...
//  Read records on card
//
//  payment: payment content pointer.
//  application_type: Application of the card, see the initial doc.
//  merchant_id: Merchant Token of a merchant, that intends to read a record
//  merchant_id_length: length should be between 13 to 32 bytes long.
//  record_idx: Record to be read on a card
//  timeout: Card present timeout
...
//  Write records to card
//
//  payment: payment content pointer.
//  application_type: Application of the card, see the initial doc.
//  merchant_id: Merchant Token of a merchant, that intends to read a record
//  merchant_id_length: lengtyh should be between 13 to 32 bytes long.
//  Record: Record to be written on a card
//  record_idx: Index of the record, we want to write to.
//  uid: uid of the card, If this is provided, and the uid on the card
//  doesn’t match to this uid, the callback will return CR_PAYMENT_RECORD_RESULT_UID_MISMATCH. If
//  this field is not populated, the record will be written to the presented card.
//  uid_length: Actual length of uid.
//  timeout: Card present timeout
...
// Although the reader-originated l2io messages are in codetalker format,
